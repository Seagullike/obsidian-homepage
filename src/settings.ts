...
export class HomepageSettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(this.containerEl)
            .setName("Interval Time(min)")
            .setDesc("Auto open homepage after interval time")
            .addText(text => {
                new suggestor(this.app, text.inputEl);
                text.setPlaceholder(DEFAULT_SETTINGS.intervalTime.toString())
                    .setValue(this.settings.intervalTime?.toString())
                    .onChange(async (value) => {
                        var valInt = parseInt(value);
                        if (Number.isNaN(valInt)) {
                            valInt = DEFAULT_SETTINGS.intervalTime;
                        }
                        if (valInt < 0) {
                            valInt = 0;
                        }
                        this.settings.intervalTime = valInt;
                        await this.plugin.saveSettings();
                        this.display();
                    });
            });
